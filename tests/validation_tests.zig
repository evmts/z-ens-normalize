...
const BASIC_VALID_TESTS = [_]ValidationTestCase{
    .{ .name = "simple_ascii", .input = "hello", .expected_script = validator.ScriptGroup.ASCII, .comment = "Simple ASCII" },
    .{ .name = "digits", .input = "123", .expected_script = validator.ScriptGroup.ASCII, .comment = "Digits" },
    .{ .name = "mixed_ascii", .input = "test123", .expected_script = validator.ScriptGroup.ASCII, .comment = "Mixed ASCII" },
    .{ .name = "with_hyphen", .input = "test-name", .expected_script = validator.ScriptGroup.ASCII, .comment = "With hyphen" },
};

// Underscore rule tests
const UNDERSCORE_TESTS = [_]ValidationTestCase{
    .{ .name = "leading_underscore", .input = "_hello", .expected_script = validator.ScriptGroup.ASCII, .comment = "Leading underscore" },
    .{ .name = "multiple_leading", .input = "____hello", .expected_script = validator.ScriptGroup.ASCII, .comment = "Multiple leading underscores" },
...
const LABEL_EXTENSION_TESTS = [_]ValidationTestCase{
    .{ .name = "valid_hyphen", .input = "ab-cd", .expected_script = validator.ScriptGroup.ASCII, .comment = "Valid hyphen placement" },
...
const FENCED_TESTS = [_]ValidationTestCase{
    ...
    .{ .name = "apostrophe_valid", .input = "hel'lo", .expected_script = validator.ScriptGroup.ASCII, .comment = "Valid apostrophe placement" },
};
...
fn runTestCase(test_case: ValidationTestCase) !void {
    ...
    if (test_case.expected_error) |expected_error| {
        ...
        if (test_case.expected_script) |expected_script| {
            try testing.expectEqual(expected_script, validated.script_group);
        }
    }
}
...
test "validation - script group detection" {
    ...
    // ASCII test
    {
        ...
        try testing.expectEqual(validator.ScriptGroup.ASCII, result.script_group);
    }
}
